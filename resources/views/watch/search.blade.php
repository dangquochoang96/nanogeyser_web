        ...
        <div class="breadcumb">
            <div class="container"> 
                <ul class="ul-bread ul-none">
                    ...
                    <li><a href="/">Tìm kiếm từ khóa: {{$key_serch}}</a></li>
                ...
                <div class="sp-dm">
                    <div class="img-sp">
                        <a href="{{route('showBySlug', ['slug' => $item->slug])}}">
                            <img src="{{(sizeof($item->productImages)) ? asset($item->productImages->first()->link) : '' }}" alt="{{$item->name}}" class="img-responsive">
                            @if ($item->best_sell == 1)
                            <span class="hot-pr">Hot</span>
                            @endif
                        </a>
                        <div class="hover-sp">
                            <a href="{{route('showBySlug', ['slug' => $item->slug])}}" class="view-sp">Xem chi tiết</a>
                            <a href="" class="view-nhanh" data-type="1" data-title="xem nhanh"
                            data-toggle="modal" data-name="{{$item->name}}" data-price='{{number_format($item->price,0,".",",")}} đ' data-link="{{route('showBySlug', ['slug' => $item->slug])}}" data-id="{{$item->id}}" data-pricenew='{{number_format($item->sale_price,0,".",",")}} đ' data-image="{{(sizeof($item->productImages)) ? asset($item->productImages->first()->link) : '' }}" data-model="{{$item->model}}" data-weight="{{$item->weight}}" data-number_filter="{{$item->number_filter}}" data-filter_technology="{{$item->filter_technology}}" data-filter_capacity="{{$item->filter_capacity}}" data-producer="{{$item->producer}}" data-ability_clean="{{$item->ability_clean}}" data-target="#myModal">Xem nhanh</a>
                        </div>
                    </div>
                    <h3><a href="{{route('showBySlug', ['slug' => $item->slug])}}">{{$item->name}}</a></h3>
                    @if ($item->sale_price)
        ...
        <div class="modal-content">
            <div class="row">
                <div class="col-md-5 col-sm-5 col-xs-12 center-box">
                    ...
                    <div class="div_success" style="display: none;color: #65A63A;">Thêm vào giỏ hàng thành công</div>
