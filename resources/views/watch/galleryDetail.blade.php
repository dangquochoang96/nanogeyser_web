...
<div class="container">
    <div class="dm-child dm-news">
        <h1 class="tit-page-d">{{ $gallery->name }}</h1>
        <div class="view-like">
            <span class="trafic"><i class="fa fa-eye"></i> Lượt xem: {{ $gallery->view }}</span>
    ...
    <div class="bg-news-lq">
        <div class="ha-lq news-lq owl-carousel owl-theme">
            @foreach($galleryRelation as $item)
            <div class="item">
                <div class="news-list">
                    <a href="{{route('gallery-detail', ['slug' => str_slug($item->name.'-'.$item->id)])}}" class="news-l-img"><img src="{{(sizeof($item->galleryImages)) ? asset($item->galleryImages->first()->link) : '' }}" alt="" class="img-responsive"></a>
