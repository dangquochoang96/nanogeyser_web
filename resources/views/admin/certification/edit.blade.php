...
@section('content')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <a href="{{ route('certifications.index') }}">Danh sách thư viện ảnh</a>
                <i class="fa fa-angle-right"></i>
            </li>
            <li>
    ...
    <div class="portlet light">
        ...
        <div class="portlet-body">
            <form method="POST" action="{{route('certifications.update', ['id' => $certification->id])}}" accept-charset="UTF-8" id="add-form" enctype="multipart/form-data">
            <div class="tabbable">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                            ...
                                            <div class="icheck-inline">
                                                <label class="control-label font-green" role="button">
                                                    <input type="radio" name="status" value="1" class="icheck"
                                                           data-radio="iradio_minimal-green"{{ $certification->status == 1 ? ' checked' : ''}}/>
                                                    Kích hoạt
                                                </label>
                                                <label class="control-label font-red-soft" role="button">
                                                    <input type="radio" name="status" value="2" class="icheck"
                                                           data-radio="iradio_minimal-green"{{ $certification->status == 2 ? ' checked' : ''}}/>
                                                    Khóa
                                                </label>
                                            </div>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="type" class="form-control">
                                            @foreach($typeLabels as $value => $label)
                                                <option value="{{ $value }}" {{ $certification->type == $value ? 'selected' : '' }}>{{ $label }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="meta" class="form-control" value="{{ $certification->meta }}"/>
                                    </div>
                                    <div class="form-group">
                                        <label class="control-label">keywords</label>
                                        <input type="text" name="keyword" class="form-control" value="{{ $certification->keyword }}"/>
                                    </div>
                                    <div class="form-group">
                                        <label class="control-label">Mô tả</label>
                                        <textarea class="form-control ckeditor" name="description">{{ $certification->description }}</textarea>
                                    </div>
...
@section('script')
    ...
    <script>
        ...
        @php
            $list = [];
        foreach ($certification->certificationImages as $value) {
            $list[] = $value->link;
        }
        @endphp
        CertificationUpload.init({
            idUpload: '#input-upload-image',
            idTable: '#table-image',
            idTarget: '#data-images',
            uploadSuccessData: {!! $certification->certificationImages !!}
        });
    </script>
@endsection
