...
@section('content')
    ...
    <div class="portlet light">
        <div class="portlet-title">
            ...
            <div class="actions">
                <a href="{{ route('certifications.create') }}" class="btn blue uppercase">
        ...
        <div class="portlet-body">
            ...
            @if (count($certifications))
                <div class="table-responsive">
                    <table class="table table-bordered table-hover td-middle">
                        ...
                        @foreach ($certifications as $certification)
                            <tr>
                                <td>{{$loop->index + ($certifications->currentPage() * $certifications->perPage()) - $certifications->perPage() + 1}}</td>
                                <td>
                                    <img src="{{(sizeof($certification->certificationImages)) ? asset($certification->certificationImages->first()->link) : '' }}"
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-danger btn-sm my-delete-btn"
                                            data-action="{{route('certifications.destroy', ['id' => $certification->id])}}">Xóa
                                    </button>
                                </td>
                            </tr>
                        @endforeach
                ...
                <div class="text-right">
                    {!!  $certifications->appends(Request::all())->links() !!}
                </div>
            @else
