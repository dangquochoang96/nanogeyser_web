...
<div class="portlet light">
  <div class="portlet-title">
     <div class="caption">
        <span class="caption-subject bold uppercase"> Danh sách pages </span>
     </div>
     <div class="actions">
        <a href="{{ URL::action('Admin\PageController@addPage') }}" class="btn blue uppercase">
           ...
           <tbody>
            <?php  $count = ($pages->currentpage() - 1) * $pages->perpage() + 1;  ?>
              ...
              <tr>
                 ...
                 <td class="text-center"><a target="_blannk" href="/{{ $page->slug }}">{{ $page->name }}</a></td>
                 <td class="text-center">{!! $page->shortdes !!}</td>
                 ...
                 <td>
                    <a href="{{ URL::action('Admin\PageController@editPage', ['id' => $page->id]) }}" class="btn btn-xs green-jungle"><i class="fa fa-pencil"></i></a>
                    <button type="button" data-id="{{ $page->id }}" class="btn-del btn btn-xs red-soft m-0"><i class="fa fa-trash"></i>
                    </button>
                 </td>
              </tr>
              @endforeach
...
<div id="del-modal" class="modal fade" tabindex="-1" data-keyboard="false" style="margin-top: 5%">
  <div class="modal-dialog modal-md">
     <form method="POST" action="{{ URL::action('Admin\PageController@delPage') }}" accept-charset="UTF-8" id="del-form">
        @csrf
        <div class="modal-content">
           <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
              <h4 class="modal-title text-uppercase">Xóa Trang</h4>
           </div>
