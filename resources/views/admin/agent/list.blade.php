...
<div class="portlet light">
  <div class="portlet-title">
     <div class="caption">
        <span class="caption-subject bold uppercase"> Danh sách đại lý </span>
     </div>
     <div class="actions">
        <a href="{{ URL::action('Admin\AgentController@add') }}" class="btn blue uppercase">
        <i class="fa fa-plus-circle"></i> Thêm
        </a>
     </div>
  </div>
  <div class="portlet-body">
    <form method="GET" action="{{ URL::action('Admin\AgentController@list') }}" class="form-inline m-b-15">
        <div class="form-group">
            <input type="text" name="name" class="form-control" placeholder="Tên đại lý" value="{{ request('name') }}">
        </div>
        <div class="form-group">
    ...
    @if(count($dealers) > 0)
     <div class="table-responsive">
        <table class="table table-bordered table-hover td-middle">
           ...
           <tbody>
            <?php $count = ($dealers->currentpage() - 1) * $dealers->perpage() + 1; ?>
              ...
              <tr>
                 ...
                 <td>
                    <a href="{{ URL::action('Admin\AgentController@edit', ['id' => $dealer->id]) }}" class="btn btn-xs green-jungle"><i class="fa fa-pencil"></i></a>
                    <button type="button" data-id="{{ $dealer->id }}" class="btn-del btn btn-xs red-soft m-0"><i class="fa fa-trash"></i></button>
                 </td>
              </tr>
              @endforeach
           </tbody>
     ...
     <div class="text-right">
      {!! $dealers->appends(Request::all())->links() !!}
    </div>
    @else
...
<div id="del-modal" class="modal fade" tabindex="-1" data-keyboard="false" style="margin-top: 5%">
  <div class="modal-dialog modal-md">
     <form method="POST" action="{{ URL::action('Admin\AgentController@del') }}" accept-charset="UTF-8" id="del-form">
        @csrf
        <div class="modal-content">
           <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
              <h4 class="modal-title text-uppercase">Xóa đại lý</h4>
           </div>
           <div class="modal-body">
              <input type="hidden" name="txt-uid" value="">
              <div class="font-red-soft">Bạn có chắc chắn muốn xóa đại lý này?</div>
           </div>
