...
<div class="portlet light">
  <div class="portlet-title">
     ...
     <div class="actions">
        <a href="{{ URL::action('Admin\MenuController@addMenu') }}" class="btn blue uppercase">
    ...
    @if(count($menus) > 0)
     <div class="table-responsive">
        <table class="table table-bordered table-hover td-middle">
           ...
           <tbody>
            <?php  $count = ($menus->currentpage() - 1) * $menus->perpage() + 1;  ?>
              ...
              <tr>
                 ...
                 <td>
                    <a href="{{ URL::action('Admin\MenuController@editMenu', ['id' => $menu->id]) }}" class="btn btn-xs green-jungle"><i class="fa fa-pencil"></i></a>
                    <button type="button" data-id="{{ $menu->id }}" class="btn-del btn btn-xs red-soft m-0"><i class="fa fa-trash"></i>
                    </button>
                 </td>
              </tr>
              @endforeach
...
<div id="del-modal" class="modal fade" tabindex="-1" data-keyboard="false" style="margin-top: 5%">
  <div class="modal-dialog modal-md">
     <form method="POST" action="{{ URL::action('Admin\MenuController@delMenu') }}" accept-charset="UTF-8" id="del-form">
        @csrf
        <div class="modal-content">
           <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
              <h4 class="modal-title text-uppercase">Xóa Menu</h4>
           </div>
           <div class="modal-body">
              <input type="hidden" name="txt-uid" value="">
              <div class="font-red-soft">Bạn có chắc chắn muốn xóa menu này?</div>
           </div>
