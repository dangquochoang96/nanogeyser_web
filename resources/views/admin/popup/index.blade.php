...
@section('content')
    ...
    <div class="portlet light">
        <div class="portlet-title">
            <div class="caption">
                <span class="caption-subject bold uppercase"> Danh sách ảnh</span>
            </div>
            <div class="actions">
                <a href="{{ route('popups.create') }}" class="btn blue uppercase">
        ...
        <div class="portlet-body">
            @if (count($popups))
                <div class="table-responsive">
                    <table class="table table-bordered table-hover td-middle">
                        ...
                        @foreach ($popups as $popup)
                            <tr>
                                <td>{{$loop->index + ($popups->currentPage() * $popups->perPage()) - $popups->perPage()}}</td>
                                <td><img src="{{$popup->image_link}}" width="320px"></td>
                                <td>{{$popup->created_at}}</td>
                                <td>
                                    <a href="{{route('popups.edit', ['id' => $popup->id])}}"
                                       class="btn btn-sm btn-info">sửa</a>
                                    <button class="btn btn-danger btn-sm my-delete-btn"
                                            data-action="{{route('popups.destroy', ['id' => $popup->id])}}">Xóa
                                    </button>
                                </td>
                            </tr>
                        @endforeach
                ...
                <div class="text-right">
                    {!!  $popups->appends(Request::all())->links() !!}
                </div>
            @else
    ...
    'deleteMessage' => 'Bạn có muốn xóa ảnh này'])
